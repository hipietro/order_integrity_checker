...
Manual creation calls `database.insert_order_into_database()`. Confirmed CSV
imports pass their validation results through
`atomic_import_service.persist_validated_orders()` to
`batch_import_repository.insert_orders_atomically()`, so every valid row is
committed as one atomic batch.

Both persistence paths apply the same storage rules:
...
This means every order-entry path follows the same normalization and
duplicate-prevention rules. The batch repository owns a separate transaction
boundary so a failed CSV row rolls back customers created for earlier rows in
